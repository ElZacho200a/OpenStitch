...
TEST_CASE("generate_concavity_cut_candidates : notch -- une seule concavite reelle -> famille bord "
          "oppose seule") {
    ...
    const bool hasValidOppositeEdge =
...
TEST_CASE("generate_concavity_cut_candidates : sablier -- deux concavites reelles -> famille "
          "concavite->concavite") {
...
TEST_CASE("select_best_concavity_cut : sablier -- choisit la coupe qui separe reellement les deux "
          "moities") {
    const auto region = hourglass_shape();
    const auto candidates = generate_concavity_cut_candidates(region);
    const auto chosen =
        select_best_concavity_cut(candidates, parametric_params(), {}, Micrometers{400});
...
TEST_CASE(
    "select_best_concavity_cut : notch -- choisit un candidat reellement construit et mesure") {
    const auto candidates = generate_concavity_cut_candidates(shape("notch"));
    const auto chosen =
        select_best_concavity_cut(candidates, parametric_params(), {}, Micrometers{400});
    REQUIRE(chosen.has_value());
...
TEST_CASE("generate_concavity_cut_candidates : sablier obstrue -- ligne droite hors de la region, "
          "coude reussit la "
          "separation") {
    ...
    const auto polygonal =
        std::find_if(candidates.begin(), candidates.end(), [](const ConcavityCutCandidate& c) {
    ...
    CHECK(polygonal->first_piece_area_mm2 + polygonal->second_piece_area_mm2 <=
          originalAreaMm2 + 0.5);
...
TEST_CASE("generate_concavity_cut_candidates : sablier obstrue, try_elbow_cuts=false -- rejet "
          "propre sans tenter de "
          "coude") {
