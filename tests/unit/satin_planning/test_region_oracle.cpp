...
auto_satin::SatinColumnsParameters parametric_params() {
    ...
    params.geometry_mode =
        auto_satin::SatinGeometryMode::Parametric; // comportement de production (autodigitize.cpp)
    return params;
}
...
TEST_CASE("evaluate_region_generation : rectangle -- couvre bien sa propre region") {
    ...
    const RegionGenerationVerdict verdict =
        evaluate_region_generation(split.regions.front(), parametric_params());
...
TEST_CASE("evaluate_decomposition_generation : boucle complete sur le corpus branche") {
    for (const std::string& name : {"t", "y", "cross", "h", "trident"}) {
        ...
        const DecompositionGenerationReport report =
            evaluate_decomposition_generation(split, parametric_params());
...
TEST_CASE(
    "evaluate_decomposition_generation : H -- le pont degrade nettement plus que les montants") {
