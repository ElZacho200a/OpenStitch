...
EdgeEndSample sample_edge_end(const SkeletonEdge& edge, bool atStart, double windowUm) {
    ...
    auto pointAt = [&](std::size_t i) -> Vec2um {
    ...
    for (std::size_t i = 0; i + 1 < n; ++i) {
        ...
        if (accumulatedLen >= windowUm)
            break;
    }

    if (!haveNear || !haveFar)
        return out; // arc degenere (longueur quasi nulle)
...
ContinuationCost continuation_cost(const SkeletonGraph& graph, std::uint32_t junctionNode,
...
JunctionPairingReport pair_branches_at_junction(const SkeletonGraph& graph,
    ...
    for (std::size_t i = 0; i < incident.size(); ++i) {
        for (std::size_t j = i + 1; j < incident.size(); ++j) {
            ...
            candidate.cost =
                continuation_cost(graph, junctionNode, incident[i], incident[j], params);
    ...
    std::stable_sort(report.candidates.begin(), report.candidates.end(),
                     [](const PairCandidate& a, const PairCandidate& b) {
                         if (a.cost.valid != b.cost.valid)
                             return a.cost.valid; // invalides en fin de liste
    ...
    if (!report.candidates.empty() && report.candidates.front().cost.valid) {
        ...
        for (auto id : incident) {
            if (id != best.edge_a && id != best.edge_b)
                report.detached.push_back(id);
        }
    } else {
...
DecompositionReport decompose_into_paths(const SkeletonGraph& graph,
    ...
    auto walkFrom = [&](std::uint32_t startEdgeId, std::uint32_t startNode) {
        ...
        while (true) {
            ...
            if (edge == nullptr)
                break; // ne devrait pas arriver, graphe coherent par construction
        ...
        if (const SkeletonNode* startN = find_node(graph, path.nodes.front()))
            path.start = startN->position;
        if (const SkeletonNode* endN = find_node(graph, path.nodes.back()))
            path.end = endN->position;
    ...
    for (const auto& edge : graph.edges) {
        ...
        if (fromContinues && toContinues)
            continue; // interne a une chaine, sera atteint depuis son vrai debut
...
std::string format_decomposition_report(const SkeletonGraph& graph,
    ...
    for (const auto& jr : report.junctions) {
        ...
        for (const auto& c : jr.candidates) {
            const bool selected = jr.selected_pair.size() == 2 && jr.selected_pair[0] == c.edge_a &&
                                  jr.selected_pair[1] == c.edge_b;
