...
MergePassReport evaluate_merge_pass(const RegionSplitReport& split, const MergePassParams& params) {
    ...
    for (const auto& candidate : split.merge_candidates) {
        ...
        if (first == nullptr || second == nullptr)
            continue; // deja filtre par split_region, garde-fou

        MergeDecision decision;
        decision.first_path_index = candidate.first_path_index;
        decision.second_path_index = candidate.second_path_index;

        const RegionGenerationVerdict firstVerdict = evaluate_region_generation(
            *first, params.genParams, params.coverageConfig, params.density);
        const RegionGenerationVerdict secondVerdict = evaluate_region_generation(
            *second, params.genParams, params.coverageConfig, params.density);
        const double firstCoverage =
            firstVerdict.coverage ? firstVerdict.coverage->raw_coverage_ratio : 0.0;
        const double secondCoverage =
        ...
        decision.separate_coverage_ratio =
            totalArea > 0.0
                ? (firstCoverage * first->area_mm2 + secondCoverage * second->area_mm2) / totalArea
                : 0.0;

        SatinRegion merged;
        merged.path_index =
            candidate.first_path_index; // convention : porte l'identite du premier des deux
        merged.region = candidate.merged_region;
        merged.area_mm2 = candidate.merged_area_mm2;
        const RegionGenerationVerdict mergedVerdict = evaluate_region_generation(
            merged, params.genParams, params.coverageConfig, params.density);
        decision.merged_build_succeeded = mergedVerdict.build_succeeded;
        decision.merged_coverage_ratio =
            mergedVerdict.coverage ? mergedVerdict.coverage->raw_coverage_ratio : 0.0;

        decision.merge_recommended =
            decision.merged_build_succeeded &&
            decision.merged_coverage_ratio >=
...
std::string format_merge_pass_report(const MergePassReport& report) {
    ...
    for (const auto& d : report.decisions) {
        out << "chemins " << d.first_path_index << "+" << d.second_path_index
            << " : separe=" << (d.separate_coverage_ratio * 100.0) << "% fusionne="
            << (d.merged_build_succeeded ? std::to_string(d.merged_coverage_ratio * 100.0) + "%"
                                         : std::string("echec"))
    ...
    out << "\nTotal : " << recommended << "/" << report.decisions.size()
        << " fusion(s) recommandee(s)\n";
    return out.str();
}
