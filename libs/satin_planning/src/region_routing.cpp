...
template <typename Column> bool column_endpoints(const Column& col, Vec2um& start, Vec2um& end) {
    if (col.rail_a.nodes.empty() || col.rail_b.nodes.empty())
        return false;
...
RegionRoutingReport route_regions(const RegionSplitReport& split,
    ...
    const stitch_generation::RoutePlan plan =
        stitch_generation::route_columns(columns, params.origin, params.routingConfig);
    report.plan = plan;
    ...
    for (auto& step : report.plan.steps) {
        if (step.column_index < columnToRegion.size())
            step.column_index = columnToRegion[step.column_index];
    }
...
std::string format_region_routing_report(const RegionRoutingReport& report) {
    ...
    for (const auto& step : report.plan.steps) {
        const char* kind = step.connector == stitch_generation::ConnectorKind::Start ? "START"
                           : step.connector == stitch_generation::ConnectorKind::Underpath
                               ? "UNDERPATH"
                               : "JUMP";
        out << "    region " << report.regions[step.column_index].path_index << " ("
            << (step.reversed ? "inverse" : "direct") << ") <- " << kind << "\n";
    }
    out << "\nTotal : deplacement=" << (report.plan.travel_um / 1000.0)
        << "mm sauts=" << report.plan.jumps << " trajets_caches=" << report.plan.underpaths
        << " liaisons_jonction=" << report.plan.junction_links << "\n";
    return out.str();
}
