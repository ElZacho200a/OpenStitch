...
struct ContinuationCost {
    double angle_cost{
        0.0}; // 0 = tangentes sortantes opposees (ligne droite), 1 = memes direction (repli)
    double width_cost{0.0}; // |log(largeurA/largeurB)|, 0 = largeurs identiques
    double curvature_cost{
        0.0}; // 0 = tangente stable sur la fenetre, plus haut = branche qui tourne
    double total{0.0};
...
[[nodiscard]] ContinuationCost continuation_cost(const SkeletonGraph& graph,
                                                 std::uint32_t junctionNode, std::uint32_t edgeA,
                                                 std::uint32_t edgeB,
...
[[nodiscard]] JunctionPairingReport
pair_branches_at_junction(const SkeletonGraph& graph, std::uint32_t junctionNode,
                          const ContinuationCostParams& params = {});
...
struct DecompositionReport {
    std::vector<JunctionPairingReport>
        junctions; // une entree par noeud Junction du graphe, tri par id croissant
